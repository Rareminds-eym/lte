...
const SECRET_ENV_KEY = "SKILLPASSPORT_INTERNAL_SECRET";
...
describe("getGatewaySecret", () => {
  it("returns the secret when it is configured and long enough", () => {
    ...
    expect(getGatewaySecret(env, SECRET_ENV_KEY)).toBe(VALID_SECRET);
  });

  it("returns a secret of exactly 32 characters", () => {
    const secret = "x".repeat(32);
    const env = { SKILLPASSPORT_INTERNAL_SECRET: secret } as unknown as LteEnv;
    expect(getGatewaySecret(env, SECRET_ENV_KEY)).toBe(secret);
  });

  it("throws FORBIDDEN when the secret is missing", () => {
    const env = {} as unknown as LteEnv;
    expect(() => getGatewaySecret(env, SECRET_ENV_KEY)).toThrow(GatewayAuthError);
    expect(() => getGatewaySecret(env, SECRET_ENV_KEY)).toThrow(
      "Gateway secret is missing or too short",
    );
  });
  ...
  it("throws FORBIDDEN when the secret is shorter than 32 characters", () => {
    ...
    try {
      getGatewaySecret(env, SECRET_ENV_KEY);
      expect.unreachable();
    } catch (error) {
