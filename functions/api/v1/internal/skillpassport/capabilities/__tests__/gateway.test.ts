...
describe("SkillPassport internal gateway registry", () => {
  it("registers capabilities:get and skills:get and derives supported actions from them", () => {
    expect(SUPPORTED_ACTIONS).toEqual(["capabilities:get", "skills:get"]);
    expect(REGISTRY["capabilities:get"]).toBe(handleCapabilitiesGet);
    expect(REGISTRY["skills:get"]).toBeDefined();
  });
});
