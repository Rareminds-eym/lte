...
export async function onRequestGet(context: PagesContext<LteEnv>): Promise<Response> {
	try {
		...
		if (context.env.SUPABASE_URL && context.env.SUPABASE_SERVICE_ROLE_KEY) {
			...
			const { data: existingUser, error } = await supabase.from("users").select("id").eq("id", user.sub).maybeSingle();
			if (error || !existingUser) {
				authLogger.warn("User data not found in LTE database during /me check", { userId: user.sub });
